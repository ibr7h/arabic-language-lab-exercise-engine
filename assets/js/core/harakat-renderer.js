...
// No MutationObserver and no legacy hand-drawn path replacement.
...
function esc(mark){ return String(mark||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
...
export function renderAttachedHaraka(mark,{anchor=50,withShadda=false}={}){
  ...
  else if(withShadda) cls='foam-mark-overlay mark-top mark-with-shadda';
  return `<span class="${cls}" data-haraka="${esc(mark)}" style="--mark-anchor:${Number(anchor)||50}%">${nativeHarakaSvg(mark)}</span>`;
}
export function renderFreeHaraka(mark){ return `<span class="free-haraka-native" data-haraka="${esc(mark)}">${nativeHarakaSvg(mark)}</span>`; }
